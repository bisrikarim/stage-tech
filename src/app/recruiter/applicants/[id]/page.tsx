              ...
              <div className="flex flex-col lg:flex-row lg:items-start gap-5">
                ...
                <div className="flex items-start gap-4 flex-1">
                  <div className="relative flex-shrink-0">
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-400 to-teal-500 flex items-center justify-center text-white text-sm font-bold">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex flex-wrap gap-1 mb-3">
                      {s.skills.map((sk) => (
                        <span key={sk} className={`text-xs px-2 py-0.5 rounded-md border font-medium ${matchedSkills.includes(sk) ? "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 border-emerald-300 dark:border-emerald-700" : "bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700"}`}>
                          {sk}
                        </span>
                      ))}
                    </div>
                    ...
                    {s.previousStages.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-2">
                        {s.previousStages.map((st) => (
                          <span key={st.company} className="flex items-center gap-1 text-xs bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800 px-2 py-1 rounded-lg">
                ...
                <div className="flex lg:flex-col items-center lg:items-end gap-3 flex-shrink-0">
                  <div className="text-center">
                    <div className={`text-2xl font-extrabold ${score >= 70 ? "text-emerald-500" : score >= 40 ? "text-amber-500" : "text-gray-400"}`}>
                      {score}%
                    </div>
                    ...
                    <div className="w-16 h-1.5 bg-gray-100 dark:bg-gray-800 rounded-full mt-1 overflow-hidden">
                      <div className={`h-full rounded-full transition-all ${score >= 70 ? "bg-emerald-500" : score >= 40 ? "bg-amber-500" : "bg-gray-300"}`} style={{ width: `${score}%` }} />
                    </div>
                  </div>
                  <div className="flex lg:flex-col gap-2">
                    <button className="flex items-center gap-1 text-xs bg-emerald-600 hover:bg-emerald-700 text-white px-3 py-1.5 rounded-lg transition whitespace-nowrap">
